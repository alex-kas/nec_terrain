...
def _validate_header(raw_hdr):
    ...
    if raw_hdr[0] != 'EFI PART':
        raise NameError("GPT partition table not found")

    # Validate the revision
    if raw_hdr[1] != _SUPPORTED_GPT_REVISION:
        raise NameError("Unsupported GPT revision '%s', supported revision " \
                         "is '%s'" % \
    ...
    if raw_hdr[2] != _GPT_HEADER_SIZE:
        raise NameError("Bad GPT header size: %d bytes, expected %d" % \
                         (raw_hdr[2], _GPT_HEADER_SIZE))

    crc = _calc_header_crc(raw_hdr)
    if raw_hdr[3] != crc:
        raise NameError("GPT header crc mismatch: %#x, should be %#x" % \
                         (crc, raw_hdr[3]))
...
class GptParser:
    ...
    def __init__(self, disk_path, sector_size = 512):
        ...
        try:
            ...
            raise NameError("Cannot open file '%s' for reading GPT " \
    ...
    def _read_disk(self, offset, size):
        ...
        try:
            ...
            raise NameError("cannot read from '%s': %s" % \
                             (self.disk_path, err))

        if len(data) != size:
            raise NameError("cannot read %d bytes from offset '%d' of '%s', " \
                             "read only %d bytes" % \
    ...
    def _write_disk(self, offset, buf):
        ...
        try:
            ...
            raise NameError("cannot write to '%s': %s" % (self.disk_path, err))
    ...
    def _read_raw_ptable(self, header):
        ...
        if crc != header['ptable_crc']:
            raise NameError("Partition table at LBA %d (%s) is corrupted" % \
    ...
    def _change_partition(self, header, entry):
        ...
        if entry['index'] >= header['entries_cnt']:
            raise NameError("Partition table at LBA %d has only %d "   \
...
def main():
    bs = 512
    gpt = GptParser(sys.argv[1], bs)
    header = gpt.read_header();
    print "%s: %s: %s"%('offset'.ljust(14), 'name'.ljust(14), 'size')
    print "%s: %s: %s"%(("0x%012x"%0).ljust(14), 'partitions'.ljust(14), ("%d"%(header['ptable_offs']+header['ptable_size'])))
    for part in gpt.get_partitions():
        print "%s: %s: %s"%(("0x%012x"%(part['first_lba']*bs)).ljust(14), part['name'].ljust(14), ("%d"%((part['last_lba']-part['first_lba']+1)*bs)))
